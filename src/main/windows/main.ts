...
export function createWindow(options?: { chatId?: string; subChatId?: string }): BrowserWindow {
  ...
  // Load the renderer - skip auth check (auth disabled for local-only operation)
  const devServerUrl = process.env.ELECTRON_RENDERER_URL

  console.log("[Main] ========== AUTH DISABLED ==========")
  console.log("[Main] 21st.dev authentication has been disabled")
  console.log("[Main] App now operates in local-only mode")
  console.log("[Main] ====================================")

  // Always load the app (auth check bypassed)
  console.log("[Main] ✓ Loading app (local-only mode)")
