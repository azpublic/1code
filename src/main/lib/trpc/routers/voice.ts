 ...
 *
 * 21st.dev backend transcription has been disabled - only local OpenAI API key is supported
 */
...
async function transcribeViaBackend(
  ...
  throw new Error("21st.dev voice transcription is disabled. Please configure an OpenAI API key in settings.")
}
...
export const voiceRouter = router({
   ...
   * DISABLED: 21st.dev backend transcription is disabled - only local OpenAI API key works
   */
  transcribe: publicProcedure
    ...
    .mutation(async ({ input }) => {
      ...
      // Only local OPENAI_API_KEY is supported (21st.dev backend disabled)
      ...
      throw new Error(
        "Voice transcription requires OPENAI_API_KEY environment variable. Set it in Settings > Models."
      )
    }),

  /**
   * Check if voice transcription is available
   * DISABLED: 21st.dev backend transcription - only local OpenAI API key works
   */
  isAvailable: publicProcedure.query(async () => {
    const hasLocalKey = !!getOpenAIApiKey()

    // Only local API key is supported (21st.dev backend disabled)
    if (hasLocalKey) {
      return {
        available: true,
    ...
    return {
      ...
      method: null,
      reason:
        "Voice transcription requires an OpenAI API key. Add it in Settings > Models.",
    }
  }),

  /**
   * Set OpenAI API key from user settings
   * This allows users to use their own API key for voice transcription
   */
