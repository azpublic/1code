...
})
{
  ...
  // Handle model change - update chat's modelId in database to prevent bleeding between chats
  const handleModelChange = useCallback((modelId: string) => {
    // Update this specific chat's modelId to prevent bleeding
    if (chatId) {
      trpcClient.chats.update.mutate({
        chatId,
        modelId,
      })
    }
